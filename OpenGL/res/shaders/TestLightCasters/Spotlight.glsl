...
void main()
{
	...
	//vector from the light pos e the frag pos
	vec3 lightDir = normalize(u_Light.Pos - FragPos);

	//spotlight
	float theta = dot(lightDir, normalize(-u_Light.LightDir));
	//smooth edge
	...
	if (intensity > 0.0)
	{
		...
		float attenuation = 1 / (u_Light.Kc + dist*u_Light.Kl + dist*dist*u_Light.Kq);
